...
export function VoteOption({ option, isSelected, onVote }: VoteOptionProps) {
  return (
    <Card className={`overflow-hidden transition-all duration-200 ${isSelected ? 'ring-2 ring-primary' : ''}`}>
      ...
      <CardContent className="p-4">
        <CardTitle className="text-lg mb-2">{option.product.name}</CardTitle>
        <div className="flex justify-between items-center">
          <span className="text-sm text-gray-500">Votes: {option._count.votes}</span>
          <Button 
